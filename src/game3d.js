...
function spawnTree() {
  ...
  for (var tries = 0; tries < 40; tries++) {
    var x = rand(BOUND.x0 + 24, BOUND.x1 - 24), y = rand(BOUND.y0 + 24, BOUND.y1 - 24);
    if (dist2(x, y, CFG.fireX, CFG.fireY) < 84 * 84) continue;   // keep a small clear zone around the fire
    var ok = true;
    for (var i = 0; i < trees.length; i++) { if (dist2(x, y, trees[i].x, trees[i].y) < 54 * 54) { ok = false; break; } }
    if (!ok) continue;
