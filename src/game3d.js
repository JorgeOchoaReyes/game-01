...
var game = {
  ...
  shake: 0, lowFuelWarn: 0, flash: 0, banner: null, tipShown: false
};
function setBanner(main, sub, dur) { game.banner = { main: main, sub: sub || '', t: dur || 2.2, max: dur || 2.2 }; }

var survivor = { x: CFG.fireX, y: CFG.fireY + 60, r: 13, face: Math.PI / 2, chill: 0, walk: 0, moving: false, chopping: false, chopT: 0 };
...
function update(dt) {
  ...
  if (near && game.carry < carryCap()) {
    ...
    if (near.chop >= CFG.gatherTime) {
      ...
      near.hit = 0.2;                                              // recoil impulse
      addParticles(near.x, near.y, 13, '#c98a4a', 130, 0.55, 3);   // chunkier chips
      addParticles(near.x, near.y, 6, '#e8c48a', 95, 0.4, 2);
      Audio2.chop(); UI.flashCarry();
      if (near.wood <= 0) {
        trees.splice(trees.indexOf(near), 1);
        addParticles(near.x, near.y, 20, '#6a9a55', 150, 0.7, 3); // felled burst
        Audio2.fell();
        setTimeout(spawnTree, 1200);
      }
      UI.syncDock();
    }
  }
  if (litByFire && dist2(survivor.x, survivor.y, CFG.fireX, CFG.fireY) < Math.pow(fireRadius() * 0.5, 2)) {
    if (game.carry > 0) { game.bank += game.carry; game.carry = 0; addParticles(CFG.fireX, CFG.fireY, 6, '#ffd24a', 70, 0.4, 3); UI.syncDock(); }
  }
...
function renderScene(dt) {
  ...
  for (var ti = 0; ti < trees.length; ti++) {
    ...
    // the pine visibly shrinks as its wood is chopped down (5 -> felled)
    ud.pine.scale.setScalar(ud.baseScale * (0.58 + 0.42 * clamp(tr.wood / 5, 0, 1)));
    ...
    var em = chopping ? 0x1e5e2c : 0x000000;
    for (var ci = 0; ci < ud.cones.length; ci++) ud.cones[ci].material.emissive.setHex(em);
    // recoil on each hit, gentle sway while chopping
    if (tr.hit > 0) tr.hit -= dt;
    ud.pine.rotation.z = tr.hit > 0 ? Math.sin(t * 55) * 0.14 : (chopping ? Math.sin(t * 30) * 0.04 : 0);
  ...
  for (var si = 0; si < shades.length; si++) {
    ...
    var by = 0.55 + Math.sin(sh.wob) * 0.12;
    ...
    if (sh.hitFlash > 0) { ud.body.material.emissive.setHex(0xffffff); ud.body.material.color.setHex(0xffffff); }
      ...
      // ease a "burning" glow: purple shade -> hot orange while inside the firelight
      sh.glow = lerp(sh.glow || 0, sh.burning ? 1 : 0, Math.min(1, dt * 8));
      var gG = sh.glow;
      ud.body.material.emissive.setRGB(lerp(0.23, 1.0, gG), lerp(0.14, 0.42, gG), lerp(0.47, 0.05, gG));
      ud.body.material.color.setRGB(lerp(0.32, 1.0, gG), lerp(0.25, 0.5, gG), lerp(0.63, 0.2, gG));
    }
...
var UI = (function () {
  ...
  hud.innerHTML =
    ...
    '<div class="woodrow"><span class="bank"></span><span class="carry"></span></div>';
  ...
  var elBank = q('.woodrow .bank'), elCarry = q('.woodrow .carry');
  ...
  function flashCarry() { elCarry.classList.remove('pop'); void elCarry.offsetWidth; elCarry.classList.add('pop'); }
  ...
  function updateHUD() {
    ...
    var cap = carryCap();
    elCarry.textContent = 'CARRYING ' + game.carry + ' / ' + cap;
    elCarry.className = 'carry' + (game.carry >= cap ? ' full' : '');
