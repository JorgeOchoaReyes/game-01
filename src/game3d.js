...
scene.add(new THREE.HemisphereLight(0x38406a, 0x0a0812, 0.5));
...
function buildShade() {
  ...
  barFg.position.set(0, 0.6, 0.001); g.add(barFg);
...
function update(dt) {
  ...
  if (near && game.carry < carryCap()) {
    ...
    if (near.chop >= CFG.gatherTime) {
      ...
      addParticles(near.x, near.y, 7, '#c98a4a', 90, 0.5, 3); Audio2.chop();
      if (near.wood <= 0) { trees.splice(trees.indexOf(near), 1); setTimeout(spawnTree, 1200); }
      UI.syncDock();
    }
  }
  ...
  var R = fireRadius(), dps = burnDps();
  for (var j = shades.length - 1; j >= 0; j--) {
    var s = shades[j]; s.wob += dt * 4;
    ...
    if (fd < R) { var burn = dps * (0.4 + fireIntensity() * 0.8) * (1 - fd / R + 0.25) * dt; s.hp -= burn; if (Math.random() < dt * 8) addParticles(s.x, s.y, 1, '#ff7b3a', 40, 0.3, 2); }
...
function renderScene(dt) {
  ...
  fireLight.intensity = (0.8 + it * 6.2) * flick;
  fireLight.distance = Math.max(6, wr(fireRadius()) * 2.6);
  ...
  for (var si = 0; si < shades.length; si++) {
    ...
    sg.lookAt(0, by, 0);
    var ud = sg.userData;
    ud.body.material.emissive.setHex(sh.hitFlash > 0 ? 0xffffff : 0x3a2478);
    ud.body.material.color.setHex(sh.hitFlash > 0 ? 0xffffff : 0x5140a0);
...
var UI = (function () {
  ...
  return { showScreen: showScreen, showTitle: showTitle, showResult: showResult, syncDock: syncDock, flashBtn: flashBtn, updateHUD: updateHUD };
})();
