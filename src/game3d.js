...
var CFG = {
  ...
  feedPerLog: 3.8, fuelCap: 130, nights: 5, nightLen: 26, dawnLen: 10, bankRadius: 92
};
...
function spawnTree() {
  // spawn in a ring around the fire, biased toward the near side so wood stays reachable
  // (still spread across the frame), off the fire/bank zone, not overlapping another tree
  var maxR = Math.min(BOUND.x1 - BOUND.x0, BOUND.y1 - BOUND.y0) * 0.5 - 24;
  for (var tries = 0; tries < 40; tries++) {
    var ang = rand(0, TAU());
    var rr = 96 + Math.pow(Math.random(), 1.7) * Math.max(70, maxR - 96);   // pow<1 skew -> closer to the fire
    var x = clamp(CFG.fireX + Math.cos(ang) * rr, BOUND.x0 + 22, BOUND.x1 - 22);
    var y = clamp(CFG.fireY + Math.sin(ang) * rr, BOUND.y0 + 22, BOUND.y1 - 22);
...
function startGame() {
  ...
  game.combo = 0; game.comboT = 0; game.comboBest = 0; game.bonus = 0; game.endless = false; game.slingCd = 0; game.biteFloatCd = 0; game.blueFire = 0; game.tkCd = 0; game.supernovaT = 0;
...
function update(dt) {
  ...
  var decay = 1.3 + game.night * 0.37 + (game.phase === 'night' ? 1.7 : 0);
  game.fuel -= decay * dt;
  ...
  else { var coldBase = 6 + game.night * 0.9 + (game.phase === 'night' ? 2.5 : 0); warmthRate = -coldBase * (1 - UPGRADES.coat.lvl * 0.12) * (game.mods.cold || 1); }
