...
function computeScore() {
  ...
  return Math.max(0, nights * 500 + game.shadesBurned * 20 + game.woodPopped * 5 + (game.state === 'win' ? 1000 : 0));
}
...
function endGame(win) {
  ...
  shades.length = 0; drops.length = 0;                 // clear the stage for the end scene
...
function update(dt) {
  ...
  for (var j = shades.length - 1; j >= 0; j--) {
    ...
    if (s.hp <= 0) {
      ...
      if (Math.random() < (s.kind === 'brute' ? 1 : 0.1 + (game.mods.drop || 0))) spawnDrop(s.x, s.y, randomDropType());
    }
  }
...
var UI = (function () {
  ...
  function showScreen(nodes, cls) {
    ...
    banner.style.opacity = 0; warn.style.opacity = 0; packhint.style.opacity = 0; stick.style.opacity = 0; buffbar.innerHTML = '';
  }
  ...
  function showResult(win) {
    ...
    var nodes = [h, p, s1, s2, s3, scoreEl, bestBadge, btn];
  ...
  return { showScreen: showScreen, showTitle: showTitle, showResult: showResult, showLevelUp: showLevelUp, syncDock: syncDock, flashBtn: flashBtn, flashCarry: flashCarry, updateHUD: updateHUD };
})();
