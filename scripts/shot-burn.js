...
const ROOT=path.join(__dirname,'..'),EXEC='/opt/pw-browsers/chromium-1194/chrome-linux/chrome',PORT=8185;
...
await pg.evaluate(()=>{const E=window.__EMBER;E.game.fuel=95;E.game.phase='night';E.game.phaseTime=20;E.game.banner=null;
  // tough shades so they linger while burning (to see the orange glow + hp bars)
  for(let i=0;i<5;i++){const a=i/5*Math.PI*2+0.3;E.shades.push({x:E.cfg.fireX+Math.cos(a)*135,y:E.cfg.fireY+Math.sin(a)*135,r:12,hp:60,maxHp:60,spd:2,biteCd:0,wob:i,hitFlash:0});}
  for(let k=0;k<14;k++)E.step(1/30);E.game.fuel=95;E.game.banner=null;});
await pg.waitForTimeout(150);await pg.screenshot({path:path.join(ROOT,'scripts/shots/52-burning.png')});
