...
const ROOT=path.join(__dirname,'..'),EXEC='/opt/pw-browsers/chromium-1194/chrome-linux/chrome',PORT=8212;
...
await pg.goto('http://localhost:'+PORT+'/index.html',{waitUntil:'load'});await pg.click('.big');await pg.waitForTimeout(200);
await pg.evaluate(()=>{const E=window.__EMBER;E.game.fuel=8;for(let k=0;k<12;k++)E.step(1/60);});
await pg.waitForTimeout(120);
const d=await pg.evaluate(()=>({dangerOpacity:getComputedStyle(document.getElementById('danger')).opacity}));
await pg.screenshot({path:path.join(ROOT,'scripts/shots/57-danger.png')});
await b.close();await new Promise(r=>server.close(r));
console.log('danger overlay opacity at fuel 8:',d.dangerOpacity,'| errors:',errs.length);
})();
