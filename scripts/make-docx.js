...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      P('A complete, tuned core loop — gather, bank, feed or upgrade, survive — across five escalating nights with clear win, lose, and restart states — and, after that first victory, an optional endless mode where the nights never stop and the score climbs until you fall — and constant, visible reward throughout: floating numbers pop from every chop, kill and feed, and a momentum combo multiplies a bonus score and rewards aggressive play with more drops. The fire visibly scales with fuel, and meters, banners and warnings keep the state legible. Progression runs on two layers: four permanent wood-bought upgrades, and a between-nights “level up” where you choose one of three run boons. Ability drops fall from enemies and each dawn — Inferno, Swift, Ward, Harvest, a screen-clearing Nova, a slingshot that flings your wood at enemies, a rare chainsaw, and a toolbelt. A tanky Brute and player-hunting irregulars — which only the slingshot or the fire itself can kill — join the shades on later nights, and the fire burns hotter and bluer the deeper you go. You roam the whole frame freely, so every tree is reachable — leaving the fire is always your own gamble. It has a persistent best score, a triumphant sunrise on a win and a bleak fade-to-dark on a loss, and fully synthesized audio. It runs entirely offline from one readable index.html plus the Three.js library.'),
